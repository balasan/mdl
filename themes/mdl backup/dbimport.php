...
function grabDesigners(){
  ...
  while ($row = mysql_fetch_assoc ($result)) {
    ...
    foreach ($row as $key => $value){
      ...
      // $splitNames = explode(" ", $postTitle);

      // $lastName = trim($lastName);

      // $match1 = glob('wp-content/uploads/newimg/designer/*'.$lastName.'*');
      // $match2 = array();
      // $match =array();

      // if(count($match1)>1){
      //   $match = glob('wp-content/uploads/newimg/designer/*'.$lastName.',*');
      //   if(count($match)>1){
      //     $match2 = glob('wp-content/uploads/newimg/designer*'.$splitNames[0].'*');
      //     $match = array_intersect($match1, $match2);
      //   }
      // }
      // else{
      //   $match=$match1;
      ...
      //   $page = get_page_by_title($postTitle, 'OBJECT','Designer');
      //   $postId = $page->ID;
      //   echo $postId. '<br>';
      //   $imgSrc = '/mdl/'.$image;
      //   addImage($imgSrc, $postId);
      ...
      // print_r(glob('wp-content/uploads/newimg/designer/*'.$lastName.'*'));
...
function grabObjects(){
  ...
  while ($row = mysql_fetch_assoc ($result)) {
    ...
    foreach ($row as $key => $value){
      ...
      // unset($matchArray);
      $count = 0;
      foreach($splitNames as $word){
        $match1 = glob('wp-content/uploads/newimg/object/*'.$word.'*');
        if(count($match1)>0){
          $matchArray[] = $match1;
        }
        $count++;
      ...
      if(count($matchArray)>1)
        $match = call_user_func_array('array_intersect',$matchArray);
      else if(count($matchArray)>0)
        $match = $matchArray[0];
      ...
      if(count($match)==1){
        ...
        // echo basename($match[$keys[0]]). '<br>';
        // echo $postTitle . '<br><br>';
        // echo $image. '<br>';

        $image = $match[$keys[0]];
        $page = get_page_by_title($postTitle, 'OBJECT','Objects');
        $postId = $page->ID;
        if(!$postId){
          echo $postTitle;
        }
